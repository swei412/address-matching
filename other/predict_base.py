...
import pickle
import os
from other.preprocessing import atomization, tokenized
...
def predict_one(address):
    ...
    global graph
    with graph.as_default():
        atom_list = atomization(address)
        address = tokenized(atom_list)
        # print('your input is:{}'.format(sentence))
        input_ids, input_mask, segment_ids, label_ids = convert(address)
...
if __name__ == "__main__":
    result = predict_one("内江市隆昌市四川内江市隆昌市李市镇新盛街2栋3单元201")
    print(result)